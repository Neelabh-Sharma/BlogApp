...
function SignInForm({ onLogin }) { // Add onLogin prop
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (login.fulfilled.match(resultAction)) {
        ...
        navigate("/dashboard", {
          state: { user: resultAction.payload.user || resultAction.payload }
        });
        
      } else {
        console.error("❌ Login failed:", resultAction.payload);
        setErrors({ 
          submit: resultAction.payload || "Login failed. Please check your credentials." 
        });
      }
    } catch (error) {
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              <div>
                ...
                <input
                  ...
                  name="email"
                  id="email"
                  className={`bg-gray-50 border text-gray-900 rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${
                    errors.email 
